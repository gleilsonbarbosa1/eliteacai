...
-- Create improved available balance function with monthly handling
CREATE OR REPLACE FUNCTION get_available_balance(p_customer_id uuid)
RETURNS decimal AS $$
BEGIN
  RETURN (
    WITH current_month_cashback AS (
      -- Get sum of valid cashback for current month only
    ...
    current_month_redemptions AS (
      -- Get sum of approved redemptions for current month only
    ...
    SELECT GREATEST(
      (SELECT total FROM current_month_cashback) - (SELECT total FROM current_month_redemptions),
      0
    )
  );
END;
...
-- Add comments
COMMENT ON FUNCTION get_available_balance IS 'Calculates available balance for current month only';
