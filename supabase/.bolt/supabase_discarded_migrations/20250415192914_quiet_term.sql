...
BEGIN
  ...
  -- Only consider cashback_amount from purchases
  UPDATE customers c
  SET balance = (
    SELECT COALESCE(SUM(
      CASE WHEN type = 'purchase' THEN cashback_amount
      ELSE -amount END
    ), 0)
    FROM transactions t
...
COMMENT ON FUNCTION handle_customer_balance() IS 'Handles customer balance updates based on transaction changes. For purchases, uses cashback_amount (3%). For redemptions, subtracts the redemption amount.';