...
-- Create improved available balance function with strict non-negative handling
...
BEGIN
  ...
  SELECT COALESCE(SUM(GREATEST(cashback_amount, 0)), 0)
  ...
  -- Calculate final balance (ensure it's never negative)
...
-- Create improved balance handling function with strict validation
...
BEGIN
  ...
  IF TG_OP IN ('INSERT', 'UPDATE') THEN
    UPDATE customers c
    SET balance = GREATEST(get_available_balance(c.id), 0),
        updated_at = NOW()
    WHERE id = NEW.customer_id;
...
-- Add comments
COMMENT ON FUNCTION get_available_balance IS 'Calculates available balance ensuring non-negative values';
COMMENT ON FUNCTION handle_customer_balance IS 'Handles balance updates and validates redemptions with strict non-negative balance';
